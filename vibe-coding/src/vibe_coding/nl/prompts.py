...
2. 禁止 import 除以下模块外的任何包：
   ...
   enum, io, contextlib,
   abc, numbers, types,
   pathlib, os, glob, fnmatch, shutil, tempfile, csv, struct,
   base64, hashlib, uuid,
   logging, warnings
3. 禁止使用 eval / exec / compile / __import__ / globals / locals / getattr / setattr / delattr / input / breakpoint
4. 沙箱已为本次执行准备一个临时工作目录（cwd），允许 open / pathlib / os.walk / glob 等读写当前目录及其子目录下的文件；仍禁止 subprocess、ctypes、网络（socket / urllib / requests / http），以及 os.system / os.popen / os.exec* / os.spawn* / os.fork
...
3. 禁止 import 除以下模块外的任何包：
   ...
   base64, hashlib, uuid,
   logging, warnings
4. 禁止 eval/exec/compile/__import__/globals/locals/getattr/setattr/delattr/input/breakpoint
5. 沙箱已为本次执行准备一个临时工作目录（cwd），允许 open / pathlib / os.walk / glob 等读写当前目录及其子目录下的文件；仍禁止 subprocess、ctypes、网络（socket / urllib / requests / http），以及 os.system / os.popen / os.exec* / os.spawn* / os.fork
6. 函数体不超过 300 行
...
3. 仍然遵守 import 白名单（json, re, math, string, textwrap, unicodedata, pprint, datetime, time, threading, collections, itertools, functools, heapq, bisect, operator, typing, dataclasses, copy, statistics, decimal, fractions, random, enum, io, contextlib, abc, numbers, types, pathlib, os, glob, fnmatch, shutil, tempfile, csv, struct, base64, hashlib, uuid, logging, warnings）和禁用内置函数列表（eval/exec/compile/__import__/globals/locals/getattr/setattr/delattr/input/breakpoint，以及 os.system/os.popen/os.exec*/os.spawn*/os.fork、subprocess.* 等危险调用）
4. 不允许改测试用例
5. 函数体不超过 300 行
"""
...
4. 仍然遵守 import 白名单（json, re, math, string, textwrap, unicodedata, pprint, datetime, time, threading, collections, itertools, functools, heapq, bisect, operator, typing, dataclasses, copy, statistics, decimal, fractions, random, enum, io, contextlib, abc, numbers, types, pathlib, os, glob, fnmatch, shutil, tempfile, csv, struct, base64, hashlib, uuid, logging, warnings）与禁用内置函数列表（eval/exec/compile/__import__/globals/locals/getattr/setattr/delattr/input/breakpoint，以及 os.system/os.popen/os.exec*/os.spawn*/os.fork、subprocess.* 等危险调用）。
5. 不允许修改测试用例。
6. 修复后函数体不超过 300 行。
