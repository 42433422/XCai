...
- cognition: 对象，含 agent.system_prompt（至少 120 字专业 prompt）及 agent.model（provider/model_name/max_tokens/temperature）
- actions: 对象，含 handlers 数组（合法值：echo/llm_md/webhook/vibe_edit/vibe_heal/vibe_code）

system_prompt 要求：
1. 不少于 120 字，明确身份、能力范围、工作步骤、输出格式、禁忌事项，面向 AI 执行；
2. **必须**在末尾固定 Markdown 章节模板，让模型生成的 README/答复有稳定结构：
   ## 用途
   ## 输入
   ## 输出
   ## 示例
...
actions.handlers 真伪约束（重要，违者视为非法输出）：
- "echo" 表示**只回显 payload，不会调用 LLM**；如果你希望员工真的用模型回答，必须写 "llm_md"，不要写 "echo"。
- "webhook" 必须搭配 actions.webhook = {"url": "..."} 一起出现，否则不要声明。
- 即使最终模板会兜底，handlers 也必须如实反映运行时实际行为；不要为了"看起来安全"而写 echo。

model 建议：provider 默认 "deepseek"，model_name 默认 "deepseek-chat"，temperature 0.2，max_tokens 与运行时实际使用一致（默认 4000）。"""
